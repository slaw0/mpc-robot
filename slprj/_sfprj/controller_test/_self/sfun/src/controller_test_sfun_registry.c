...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_controller_test_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_controller_test_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_controller_test_autoinheritance_info( int nlhs, mxArray
  ...
  if (strcmp(machineName, "controller_test") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_controller_test_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_controller_test_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "controller_test") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_controller_test_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_controller_test_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"controller_test")) {
    return(sf_controller_test_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void controller_test_terminator(void);
...
extern void controller_test_initializer(void);
extern void controller_test_register_exported_symbols(SimStruct* S);
extern void controller_test_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  controller_test_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      controller_test_debug_initialize();
    }

    controller_test_initializer();
  }
