...
static SFc7_trajectory_testInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c7_trajectory_test(void);
static void initialize_params_c7_trajectory_test(void);
static void enable_c7_trajectory_test(void);
static void disable_c7_trajectory_test(void);
static void finalize_c7_trajectory_test(void);
static void sf_c7_trajectory_test(void);
...
static void sf_save_state_c7_trajectory_test(FILE *c7_file);
static void sf_load_state_c7_trajectory_test(FILE *c7_file);
...
static void sf_save_state_c7_trajectory_test(FILE *c7_file)
{
  fwrite(&chartInstance.c7_is_active_c7_trajectory_test, 1,
   sizeof(chartInstance.c7_is_active_c7_trajectory_test), c7_file);
}

static void sf_load_state_c7_trajectory_test(FILE *c7_file)
{
  fread(&chartInstance.c7_is_active_c7_trajectory_test, 1,
   sizeof(chartInstance.c7_is_active_c7_trajectory_test), c7_file);
}
...
static void mdlSetWorkWidths_c7_trajectory_test(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("trajectory_test",7);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("trajectory_test",7,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"trajectory_test",7,3);
      sf_mark_chart_reusable_outputs(S,"trajectory_test",7,6);
    }
    if (!sf_is_chart_instance_optimized_out("trajectory_test",7)) {
      int dtId;
      char *chartInstanceTypedefName =
        sf_chart_instance_typedef_name("trajectory_test",7);
...
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "trajectory_test", 7);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void sf_save_debug_c7_trajectory_test(SFc7_trajectory_testInstanceStruct*
...
static void sf_load_debug_c7_trajectory_test(SFc7_trajectory_testInstanceStruct*
 chartInstance,FILE* file)
{
  int machineNumber = _trajectory_testMachineNumber_;
  int chartNumber = chartInstance->chartNumber;
...
static void sf_c7_trajectory_test_sim_ctx_io(SimStruct* S, const char io, FILE*
 file)
{
  if(io == 'r') {
    sf_load_state_c7_trajectory_test(file);
    sf_load_debug_c7_trajectory_test(&chartInstance, file);
  } else {
    sf_save_state_c7_trajectory_test(file);
    sf_save_debug_c7_trajectory_test(&chartInstance, file);
  }
}
static void mdlStart_c7_trajectory_test(SimStruct *S)
{
  ...
  chartInstance.chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c7_trajectory_test;
  chartInstance.chartInfo.restoreLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.restoreBeforeLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.storeCurrentConfiguration = NULL;
  chartInstance.S = S;
  ssSetUserData(S,(void *)(&(chartInstance.chartInfo))); /* register the chart instance with simstruct */
  ssSetmdlSimulationContextIO(S, sf_c7_trajectory_test_sim_ctx_io);
  if(!sim_mode_is_rtw_gen(S)) {
