...
static SFc4_trajectory_testInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c4_trajectory_test(void);
static void initialize_params_c4_trajectory_test(void);
static void enable_c4_trajectory_test(void);
static void disable_c4_trajectory_test(void);
static void finalize_c4_trajectory_test(void);
...
static void sf_save_state_c4_trajectory_test(FILE *c4_file);
static void sf_load_state_c4_trajectory_test(FILE *c4_file);
...
static void sf_save_state_c4_trajectory_test(FILE *c4_file)
{
  fwrite(&chartInstance.c4_is_active_c4_trajectory_test, 1,
   sizeof(chartInstance.c4_is_active_c4_trajectory_test), c4_file);
}

static void sf_load_state_c4_trajectory_test(FILE *c4_file)
{
  fread(&chartInstance.c4_is_active_c4_trajectory_test, 1,
   sizeof(chartInstance.c4_is_active_c4_trajectory_test), c4_file);
}
...
static void mdlSetWorkWidths_c4_trajectory_test(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("trajectory_test",4);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("trajectory_test",4,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"trajectory_test",4,8);
      sf_mark_chart_reusable_outputs(S,"trajectory_test",4,3);
    }
    if (!sf_is_chart_instance_optimized_out("trajectory_test",4)) {
      int dtId;
      char *chartInstanceTypedefName =
        sf_chart_instance_typedef_name("trajectory_test",4);
...
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "trajectory_test", 4);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void sf_save_debug_c4_trajectory_test(SFc4_trajectory_testInstanceStruct*
...
static void sf_load_debug_c4_trajectory_test(SFc4_trajectory_testInstanceStruct*
 chartInstance,FILE* file)
{
  int machineNumber = _trajectory_testMachineNumber_;
  int chartNumber = chartInstance->chartNumber;
...
static void sf_c4_trajectory_test_sim_ctx_io(SimStruct* S, const char io, FILE*
 file)
{
  if(io == 'r') {
    sf_load_state_c4_trajectory_test(file);
    sf_load_debug_c4_trajectory_test(&chartInstance, file);
  } else {
    sf_save_state_c4_trajectory_test(file);
    sf_save_debug_c4_trajectory_test(&chartInstance, file);
  }
}
static void mdlStart_c4_trajectory_test(SimStruct *S)
{
  ...
  chartInstance.chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c4_trajectory_test;
  chartInstance.chartInfo.restoreLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.restoreBeforeLastMajorStepConfiguration = NULL;
  chartInstance.chartInfo.storeCurrentConfiguration = NULL;
  chartInstance.S = S;
  ssSetUserData(S,(void *)(&(chartInstance.chartInfo))); /* register the chart instance with simstruct */
  ssSetmdlSimulationContextIO(S, sf_c4_trajectory_test_sim_ctx_io);
  if(!sim_mode_is_rtw_gen(S)) {
