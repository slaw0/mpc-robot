...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_MPC_framework_process_check_sum_call( int nlhs, mxArray
   * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_MPC_framework_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_MPC_framework_autoinheritance_info( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_MPC_framework_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
...
extern unsigned int sf_MPC_framework_method_dispatcher(SimStruct *S, const char
...
{
  if(sf_MPC_framework_method_dispatcher(simstructPtr,chartName,method,data))
  return 1;
  return 0;
}
extern void MPC_framework_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    MPC_framework_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void MPC_framework_initializer(void);
extern void MPC_framework_register_exported_symbols(SimStruct* S);
extern void MPC_framework_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  MPC_framework_register_exported_symbols(S);
}
